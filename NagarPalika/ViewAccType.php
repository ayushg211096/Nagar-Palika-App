...
<html class=" ">
    <head>
	   ...
	   require_once 'condb.php';
		...
		$sqlp="SELECT * FROM `acctype` WHERE `id` = '".$ac."';";	
		$res2=mysqli_query($con,$sqlp);
		if (mysqli_num_rows($res2) > 0) {
		$AccDetail=mysqli_fetch_array($res2);
		}
		
	   $pheadtitle=$AccDetail['Name']." - Account Type - View Accounts";
                        ...
                        <section class="box purple">
                            ...
                            <div class="content-body">    <div class="row">
									...
									<h2>Account Detail</h2>
										...
										<tr><th>Name :</th><td><?php echo $AccDetail['Name']; ?></td></tr>
										<tr><th>First Month Installment :</th><td><?php echo $AccDetail['FMI']; ?></td></tr>
										<tr><th>Other Month Installment :</th><td><?php echo $AccDetail['OMI']; ?></td></tr>
										</table>
										...
										<table id="example" class="table table-striped dt-responsive display" cellspacing="0" width="100%">
                                            ...
                                            <tbody>
											<?php
												$sqlp="SELECT accounts.id 'acid',accounts.slipno, accounts.name, accounts.fathername, accounts.address, accounts.mobile, accounts.smobile, accounts.remarks, accounts.AccType, agent.Name 'Agent' FROM `accounts`JOIN agent on accounts.dealer = agent.id  WHERE accounts.AccType = '".$ac."';";
												$i=0;
												$res2=mysqli_query($con,$sqlp);
												if (mysqli_num_rows($res2) > 0) {
